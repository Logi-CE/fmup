...
class SessionTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $reflection = new \ReflectionMethod('\FMUP\Session', '__construct');
        $this->assertTrue($reflection->isPrivate());

        $reflection = new \ReflectionMethod('\FMUP\Session', '__clone');
        $this->assertTrue($reflection->isPrivate());

        $reflector = new \ReflectionClass('\FMUP\Session');
    ...
    {
        $session = $this->getMockBuilder('\Tests\SessionMock')->setMethods(array('isStarted'))->getMock();
        $session->method('isStarted')->willReturn(false);
        $reflection = new \ReflectionProperty('\FMUP\Session', 'instance');
    ...
    {
        $session = $this->getMockBuilder('\Tests\SessionMock')
    ...
    {
        $session = $this->getMockBuilder('\Tests\SessionMock')->setMethods(array('isStarted', 'sessionName'))->getMock();
        $session->method('isStarted')->willReturn(true);
    ...
    public function testSetNameFails()
    {
        ...
        $reflection->setAccessible(true);
        $reflection->setValue($session);
        $this->setExpectedException('\FMUP\Exception', 'Session name could not contain only numbers');
        /** @var $session \FMUP\Session */
        $session->setName('123');
    }
    ...
    {
        $session = $this->getMockBuilder('\Tests\SessionMock')->setMethods(array('isStarted', 'sessionId'))->getMock();
        $session->method('isStarted')->willReturn(true);
    ...
    {
        $session = $this->getMockBuilder('\Tests\SessionMock')->setMethods(array('phpVersion', 'sessionId'))->getMock();
    ...
    {
        $session = $this->getMockBuilder('\Tests\SessionMock')->setMethods(array('phpVersion', 'sessionStatus'))->getMock();
    ...
    {
        $session = $this->getMockBuilder('\Tests\SessionMock')->setMethods(array('start'))->getMock();
    ...
    {
        $sapi = $this->getMockBuilder('\Tests\SapiMockSession')->setMethods(array('get'))->getMock();
        $sapi->method('get')->willReturn(\FMUP\Sapi::CGI);
        $session = $this->getMockBuilder('\Tests\SessionMock')->setMethods(array('sessionStart'))->getMock();

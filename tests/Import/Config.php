...
class ConfigTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $yes = $this->getMockBuilder('\FMUP\Import\Config\Field')
            ->setMethods(array('validateField'))
            ->disableOriginalConstructor()
            ->getMock();
        $yes->expects($this->once())->method('validateField')->willReturn(true);
        $no = $this->getMockBuilder('\FMUP\Import\Config\Field')
    ...
    public function testValidateObjects()
    {
        ...
        $mockModel = $this->getMockBuilder('\stdClass')->setMethods(array('setAttribute', 'findFirst'))->getMock();
        ...
        $configObj = $this->getMockBuilder('\FMUP\Import\Config\ConfigObjet')
            ->setMethods(
                array(
        ...
        $configObj->method('getNomObjet')->willReturn('\Tests\Import\ModelMockImportConfig');
        $config = $this->getMockBuilder('\FMUP\Import\Config')
        ...
        $config->expects($this->exactly(3))
            ...
            ->willReturn($mockModel)
            ->with($this->equalTo('\Tests\Import\ModelMockImportConfig'));
    ...
    {
        $instanceFound = $this->getMockBuilder('\stdClass')->setMethods(array('getId'))->getMock();
        ...
        $field2 = $this->getMockBuilder('\FMUP\Import\Config\Field')
            ...
            ->disableOriginalConstructor()
            ->getMock();
        $field2->method('getTableCible')->willReturn('\Tests\Import\ModelMockImportConfig');
        ...
        $configObj->method('getListeIndexChamp')->willReturn(array($field, $field));
        $configObj->method('getNomObjet')->willReturn('\Tests\Import\ModelMockImportConfig');
        $configObj->method('getIdNecessaire')->willReturn(array('\Tests\Import\ModelMockImportConfig'));
        $configObj->method('getNomAttribut')->willReturn(array('\Tests\Import\ModelMockImportConfig' => 'nom'));
        $config = $this->getMockBuilder('\FMUP\Import\Config')
            ->setMethods(
    ...
    public function testInsertLineFailsOnSave()
    {
        $instanceFound = $this->getMockBuilder('\stdClass')->setMethods(array('getId'))->getMock();
        $instanceFound->method('getId')->willReturn(2);
        $mockModel = $this->getMockBuilder('\stdClass')->setMethods(array('findFirst', 'save', 'getErrors'))->getMock();
        ...
        $config->expects($this->once())
            ...
            ->with($this->equalTo('\Tests\Import\ModelMockImportConfig'));
        /** @var $config \FMUP\Import\Config */
        $this->setExpectedException('\FMUP\Import\Exception', "error 1;error 2");
        $config->insertLine();
    }
    ...
    public function testToString()
    {
        ...
        $field->method('getValue')->willReturn('value');
        $config = $this->getMockBuilder('\FMUP\Import\Config')->setMethods(array('getListeField'))->getMock();
    ...
    {
        $method = new \ReflectionMethod('\FMUP\Import\Config', 'sortByPriority');
        $method->setAccessible(true);
        $config = new \FMUP\Import\Config;
        $comparingA = $this->getMockBuilder('\stdClass')->setMethods(array('getPriorite'))->getMock();
        $comparingA->method('getPriorite')->willReturnOnConsecutiveCalls(1, 2, 2, 3, 3);
        $comparingB = $this->getMockBuilder('\stdClass')->setMethods(array('getPriorite'))->getMock();
